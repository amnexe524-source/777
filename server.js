...
function getRamInfo() {
  ...
  if (meminfo) {
    ...
    const reclaimable = meminfo.SReclaimable ?? 0;
    const shmem = meminfo.Shmem ?? 0;
    const usedLinux = Math.max(0, total - free - buffers - cached - reclaimable + shmem);
    const clampedAvailable = clamp(available, 0, total);
    const used = Math.max(0, usedLinux);
...
function getMetricsPayload() {
  ...
  return {
    ...
    ram: {
      ...
      cachedGb: bytesToGb(ram.details?.cachedBytes ?? 0),
      buffersGb: bytesToGb(ram.details?.buffersBytes ?? 0),
