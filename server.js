...
let previousCpuTimes = readCpuTimesSafe();

function readCpuTimesSafe() {
  try {
...
function fallbackCpuFromLoadAvg() {
  const loads = os.loadavg();
  const oneMinuteLoad = Array.isArray(loads) ? loads[0] : 0;
  const cores = os.cpus()?.length || 1;
  const estimate = (oneMinuteLoad / cores) * 100;
  return Number(Math.max(0, Math.min(100, estimate)).toFixed(2));
}

function getSystemCpuUsage() {
  const current = readCpuTimesSafe();

  if (!current || !previousCpuTimes) {
    previousCpuTimes = current;
    return fallbackCpuFromLoadAvg();
  }
  ...
  if (totalDiff <= 0) return fallbackCpuFromLoadAvg();
